...
def award_program_certificates(self, username):
    ...
    """
    def _retry_with_custom_exception(user, reason, countdown):
    ...
    if not CredentialsApiConfig.current().is_learner_issuance_enabled:
        ...
        raise _retry_with_custom_exception(user=username, reason=error_msg, countdown=countdown)
        ...
        for program_uuid in new_program_uuids:
            ...
            try:
                ...
                if exc.response.status_code == 429:  # pylint: disable=no-member
                    ...
                    raise _retry_with_custom_exception(user=username, reason=error_msg, countdown=rate_limit_countdown)
                else:
                    LOGGER.exception(
...
def award_course_certificate(self, username, course_run_key):
    ...
    """
    def _retry_with_custom_exception(user, course_run_key, reason, countdown):
        exception = MaxRetriesExceededError(
    ...
    try:
        ...
        LOGGER.exception(error_msg)
        _retry_with_custom_exception(user=username, course_run_key=course_run_key, reason=error_msg, countdown=countdown)
...
def revoke_program_certificates(self, username, course_key):
    ...
    """
    def _retry_with_custom_exception(user, course_key, reason, countdown):
        exception = MaxRetriesExceededError(
    ...
    if not CredentialsApiConfig.current().is_learner_issuance_enabled:
        ...
        LOGGER.warning(error_msg)
        raise _retry_with_custom_exception(user=username, course_key=course_key, reason=error_msg, countdown=countdown)
    ...
    if program_uuids_to_revoke:
        try:
            ...
            LOGGER.exception(error_msg)
            # Retry because a misconfiguration could be fixed
            raise _retry_with_custom_exception(user, course_key, exception=exc, countdown=countdown)
