...
class EffortEstimationTransformer(BlockStructureTransformer):
    ...
    def collect(cls, block_structure):
        ...
        Pooled estimates higher on verticals and such is done by transform() below, because what each user sees could
        be different.
        """
    ...
    def _collect_video_effort(cls, block_structure, block_key, xblock, cache):
        ...
        # this course.
        duration = max(0, cache[cls.CACHE_VIDEO_DURATIONS].get(xblock.edx_video_id, 0))
        if not duration:
            print("MIKE did not find video duration info", xblock.location, xblock.edx_video_id)
            # FIXME raise cls.MissingEstimationData()
            duration = 360
    ...
    def _estimate_video_effort(self, _usage_info, block_structure, block_key):
        ...
        global_speed = block_structure.get_xblock_field(block_key, 'global_speed')
        only_on_web = block_structure.get_xblock_field(block_key, 'only_on_web')
