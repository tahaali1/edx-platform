...
def _unset_course_discussion_blackout(course, user_id):
    """
    Unset the blackout period for course discussions.
    :param course:
    :param user_id:
...
class CreateThreadTest(
    ...
    def test_basic_in_blackout_period(self):
        ...
        with self.assertRaises(DiscussionBlackedOutException) as assertion:
            create_thread(self.request, self.minimal_data)
        self.assertEqual(assertion.exception.status_code, 403)
        self.assertEqual(assertion.exception.detail, "Discussions are in black out period.")
        _unset_course_discussion_blackout(course=self.course, user_id=self.user.id)
